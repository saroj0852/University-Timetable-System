// scripts/overlap_fixer.js
// This script checks for pre-assigned conflicts in 'data/data.json'.
...
function findAssignedConflicts() {
    ...
    try {
        // UPDATED PATH
        timetable = JSON.parse(fs.readFileSync('data/data.json', 'utf8'));
    } catch (error) {
        console.error("❌ Error reading data/data.json. Make sure the file exists and is valid JSON.", error);
        return;
    }
    ...
    if (conflictCount === 0) {
        ...
        \nACTION: Please fix these directly in your 'data/data.json' file and run the Python solver again.`);
    }
}
